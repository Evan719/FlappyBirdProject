...
function CoralCollection() {
    ...
    this.add = function () {
        ...
        console.log(this._corals.length);
    };
    ...
    function ScoreUp() {
        prevScore += 1;
        document.getElementById("myscore").innerHTML = prevScore + currentScore;
    }
...
// intersection
        ...
        var cy2 = Math.min(Math.max(fish.y, this.y + this.height + 120), this.y + 2 * this.height + 80);
// Closest difference
        var dx = fish.x - cx;
        var dy1 = fish.y - cy1;
        var dy2 = fish.y - cy2;
...
function loadGraphics() {
    ...
    var img = new Image();
    img.src = "images/sheetcopy2.png";
    img.onload = function () {
        initSprites(this);
        renderingContext.fillStyle = backgroundSprite.color;

        okButton = {
            x: (width - okButtonSprite.width) / 2.2,
            y: height - 200,
...
function update() {
    ...
    if (currentState !== states.Score) {
        foregroundPosition = (foregroundPosition - 2) % 150; // Move left two px each frame. Wrap every 14px.
    }

    if (currentState !== states.Score) {
        backgroundPosition = (backgroundPosition - 2) % 250; // Move left two px each frame. Wrap every 14px.
    }
...
function splash() {
    ...
    renderingContext.fillRect(40, 320, 10, 10);
    renderingContext.font = "20px bolder";
    renderingContext.fillText("Tap To Start/Fly!", 57, 335);
